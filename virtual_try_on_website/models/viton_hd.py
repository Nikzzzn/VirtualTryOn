...
class SegGenerator(BaseNetwork):
    def __init__(self, input_nc, output_nc=13, norm_layer=nn.InstanceNorm2d):
...
class VITON_HD_GMM(nn.Module):
    def __init__(self, inputA_nc, inputB_nc):
        super(VITON_HD_GMM, self).__init__()
        ...
        self.regression = FeatureRegression(input_nc=(load_width // 64) * (load_height // 64),
...
class ALIASResBlock(nn.Module):
    def __init__(self, input_nc, output_nc, use_mask_norm=True):
        ...
        subnorm_type = 'spectralaliasinstance'
...
class ALIASGenerator(BaseNetwork):
    def __init__(self, input_nc):
        ...
        self.head_0 = ALIASResBlock(nf * 16, nf * 16)

        self.G_middle_0 = ALIASResBlock(nf * 16 + 16, nf * 16)
        self.G_middle_1 = ALIASResBlock(nf * 16 + 16, nf * 16)

        self.up_0 = ALIASResBlock(nf * 16 + 16, nf * 8)
        self.up_1 = ALIASResBlock(nf * 8 + 16, nf * 4)
